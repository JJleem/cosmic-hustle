...
${combined}`,
    ...
    const finalSummary = res.content[0].text;
...
async function summarizeTextChunk(text, index, total) {
    process.stdout.write(`  청크 요약 중... (${index + 1}/${total})\r`);
    const res = await client.messages.create({
        model: MODEL,
        max_tokens: 600,
...
function buildFinalPrompt(content) {
...
function parseFinalSummary(res, chunkCount) {
    const finalSummary = res.content[0].text;
    const tagsMatch = finalSummary.match(/## Tags\s*\n([^\n#]+)/);
    const tags = tagsMatch
        ? tagsMatch[1].split(',').map((t) => t.trim()).filter(Boolean)
