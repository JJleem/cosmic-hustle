...
백엔드 작업 시 `backend/` 디렉토리에서 Codex 실행 → `backend/AGENTS.md` 로드됨.

---

## 현재 전환 목표: vault 기반 멀티에이전트 운영실

`/dashboard`는 관측/제어 화면이고, 본체는 에이전트들이 자동으로 대화·작업·기록하는 멀티에이전트 시스템이다.

이 목표와 관련된 작업을 시작하면, 코드 수정 전에 Obsidian vault를 먼저 읽어 현재 세션 맥락을 복구할 것.
...
knowledge/session-memory-automation.md
logs/ 의 최신 handoff
필요한 agents/*.md
```

Vault가 장기기억의 source of truth다. Hermes built-in memory는 보조 기억으로만 취급한다.
...
cosmic-hustle/
├── web/          # Next.js 프론트엔드
├── backend/      # Python FastAPI (상세 → backend/AGENTS.md)
└── AGENTS.md
```
